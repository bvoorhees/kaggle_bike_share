...
train <- read.csv("./raw_data/train.csv") #use nrows=1000 rows for speed during feature engineering
test <- read.csv("./raw_data/test.csv") #use nrows=1000 rows for speed during feature engineering
...
casualFit <- randomForest(casual ~ hour + year + humidity + temp + atemp + workingday + weekday, 
